...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const entries = getRecentLogs(lines, level);
  if (entries.length === 0) {
    await interaction.editReply({ content: "No logs available yet." });
    return;
  }

  const embed = new EmbedBuilder()
    .setColor(Colors.Blue)
    .setTitle(`Logs (last ${entries.length})`)
  ...
  for (const e of entries) {
    ...
    const line = `${icon} ${new Date(e.timestamp).toLocaleTimeString()} [${e.level}] ${e.message.slice(0, 150)}`;
    description += line + "\n";
