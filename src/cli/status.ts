...
import { getQueueSnapshot } from "../opencode/engine.js";
...
export async function printStatus(): Promise<void> {
  ...
  if (config) {
    ...
    console.log(`  Queue       ${queue.filter((q) => q.status === "queued").length} queued / ${queue.filter((q) => q.status === "running").length} running`);
